...
class GCNPerturb(GNN_basic):
    ...
    def get_layers(self):
        ...
        for l in range(self.num_layers):
            self.convs.append(GCNConv(current_dim, self.hidden_dim, normalize=False))
    ...
    def forward_prediction(self, x):
        ...
        # node embedding for GNN
        emb, self.P = self.get_emb_prediction(x)
        x = self.readout_layer(emb, batch)
        self.logits = self.mlps(x)
        self.probs = F.log_softmax(self.logits, dim=1)
        return self.probs, self.P
    ...
    def get_emb(self, *args, **kwargs):
        ...
        self.sub_adj = from_edge_index_to_adj_torch(
            edge_index, edge_attr, self.num_nodes
        )
        ...
        new_edge_index, new_edge_attr = from_adj_to_edge_index_torch(norm_adj)
