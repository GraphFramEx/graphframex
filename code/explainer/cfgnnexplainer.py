...
from gnn.gcn_perturb import GCNPerturb
...
class CFExplainer:
    ...
    def __init__(
        ...
        input_dim, output_dim = self.sub_feat.shape[1], self.num_classes
        # Instantiate CF model class, load weights from original model
        self.cf_model = GCNPerturb(
            input_dim, output_dim, self.sub_adj, cf_model_params, beta
        )
    ...
    def train(self, epoch):
        ...
        output = self.cf_model.forward(**{"x": self.x, "adj": self.A_x})
        ...
        if y_pred_new_actual != self.y_pred_orig:
            ...
            perturb = np.abs(cf_adj.detach().numpy() - self.sub_adj.detach().numpy())
