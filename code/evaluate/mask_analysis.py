...
def get_mask_properties(args):
    ...
    mask_filename = get_mask_path(args) + args.dataset + '_' + args.explainer_name + '_model_' + args.model + '_phenfocus_' + str(args.true_label_as_target) + '_test_' + str(args.num_test) + '_seed' + str(args.seed) + '.pkl'
    ...
    for i, node_index in enumerate(node_indices):
        subgraph, relabeled_node_index = from_mask_to_nxsubgraph(edge_masks[i], node_index, data)
        connected_subgraph = subgraph.subgraph(nx.node_connected_component(subgraph.to_undirected(), relabeled_node_index))
        edge_weights = nx.get_edge_attributes(subgraph, "weight")
        print('is subgraph connected:', nx.is_weakly_connected(subgraph))
        print('is connected subgraph connected:', nx.is_weakly_connected(connected_subgraph))
        ...
        centralities = nx.information_centrality(connected_subgraph.to_undirected(), weight="weights")
        ...
        mask_property = {'dataset': args.dataset, 
