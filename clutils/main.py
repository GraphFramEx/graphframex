...
if not os.path.exists("/cluster/scratch/kamara/checkpoints"):
    os.makedirs("/cluster/scratch/kamara/checkpoints")
...
if args.command == "sweep":
    ...
    if "machine" in config:
        ...
        if ("mem" in config["machine"]) & ("gpus-per-task" in config["machine"]):
            sbatch += "-R 'rusage[mem=%s,ngpus_excl_p=%s]' " % (
                config["machine"]["mem"],
                config["machine"]["gpus-per-task"],
            )
        else:
    ...
    params_to_index = [
        ...
        if len(values) >= 2
        and (any(["/" in v for v in values if type(v) is str]) or any([len(str(v)) >= 20 for v in values]))
    ]
    ...
    if args.array:
        ...
        with open(filename, "w") as f:
            f.write(
                cmdPre(
                    config, None, name, log_stdout, log_stderr, filename, num_expes=len(paramset), pooling=args.pooling
                )
            )
