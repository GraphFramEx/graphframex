...
from code.utils.io_utils import save_checkpoint
...
def main(args):
    ...
    infos = {
        ...
        "groundtruth target": is_true_label,
    ...
    if args.dataset != "syn2":
        accuracy_top = eval_accuracy(
            data, edge_masks, list_test_nodes, args, top=True, num_top_edges=args.num_top_edges
        )
    ...
    probs_summary = {
        "ori_probs": ori_probs.mean().item(),
        "unimportant_probs": unimportant_probs.mean().item(),
        "important_probs": important_probs.mean().item(),
    }
